...
**使用步骤**：
1. 调 `kb_upload.py` **之前**先输出一行确认，**明确分开写出目标来源名与库名**（多来源下防"传错空间"的关键——见[来源感知与提示](#来源感知与提示多来源防错)）：
   > "确认：即将把《<title>》上传到来源 **`<来源名>`** 的 **`<库名>`** 库...（取消请说"停"，或指定别的来源/库）"
2. 等一拍让用户反应，未取消则执行
...
多来源场景下用户易忘记"当前在哪个空间"。**不要每次机械提示当前来源**（对长期单来源或熟练用户是噪音、费 token）——只在以下三个高价值时刻把来源说清楚：

- **上传确认时（最高优先级）**：确认行必须明确写出目标 `来源名 + 库名`（见能力 4），防误传到错的空间——上传代价高、难挽回。
- **切换默认来源后**：执行 `--set-default-source` 后，自然回一句"默认空间已切到 **X**，后续无 `--source` 的检索/上传都走这里"——只在切换时提，不每次。
- **多源检索结果**：每个 chunk 已自带 `source` 标签；当检索**跨多个来源**时，用一句话点明"结果来自 A、B 两个空间"，帮用户快速定位。

> 优先依赖检索结果自带的 `source` 标签（最优雅的隐式提示），而非额外弹"当前空间"提示。
